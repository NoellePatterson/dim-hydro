...
def dim_hydrograph_plotter_agg(start_date, directory_name, end_with, class_number, gauge_numbers, plot):
    aggregate_matrix = np.zeros((366, 7))
    counter = 0
...
def _getAggMatrix(flow_matrix):
    ...
    for row_index, row_data in enumerate(flow_matrix[:,0]):
        ...
        percentiles[row_index,0] = np.nanmin(normalized_matrix[row_index,:])
        percentiles[row_index,1] = np.nanpercentile(normalized_matrix[row_index,:], 10)
        percentiles[row_index,2] = np.nanpercentile(normalized_matrix[row_index,:], 25)
        percentiles[row_index,3] = np.nanpercentile(normalized_matrix[row_index,:], 50)
        percentiles[row_index,4] = np.nanpercentile(normalized_matrix[row_index,:], 75)
        percentiles[row_index,5] = np.nanpercentile(normalized_matrix[row_index,:], 90)
